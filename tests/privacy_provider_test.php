...
use enrol_bkash\privacy\provider;
use core_privacy\local\request\writer;

/**
 * Class enrol_bkash_privacy_provider_testcase.
 *
 * @package    enrol_bkash
 * @copyright  2021 Brain station 23 ltd.
 * @author     Brain station 23 ltd.
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class enrol_bkash_privacy_provider_testcase extends \core_privacy\tests\provider_testcase {

    /** @var stdClass A user whose email address matches the business field in some of the bkash transactions. */
    ...
    /** @var stdClass A user whose email address matches the receiver_email field in some of the bkash transactions. */
    ...
    public function test_get_metadata() {
        $collection = new collection('enrol_bkash');
        $newcollection = provider::get_metadata($collection);
        $itemcollection = $newcollection->get_collection();
        $this->assertCount(2, $itemcollection);

        $location = reset($itemcollection);
        $this->assertEquals('bkash.com', $location->get_name());
        $this->assertEquals('privacy:metadata:enrol_bkash:bkash_com', $location->get_summary());
        ...
        $this->assertEquals('enrol_bkash', $table->get_name());
        $this->assertEquals('privacy:metadata:enrol_bkash:enrol_bkash', $table->get_summary());
    ...
    public function test_export_user_data() {
        ...
        $this->export_context_data_for_user($this->student1->id, $coursecontext1, 'enrol_bkash');
    ...
    public function test_export_user_data_not_enrolled() {
        ...
        // Export all of the data for the context.
        $this->export_context_data_for_user($this->student2->id, $coursecontext1, 'enrol_bkash');
        $writer = writer::with_context($coursecontext1);
    ...
    public function test_export_user_data_no_enrolment() {
        ...
        // Export all of the data for the context.
        $this->export_context_data_for_user($this->student0->id, $coursecontext1, 'enrol_bkash');
        $writer = writer::with_context($coursecontext1);
        $this->assertFalse($writer->has_any_data());
    }

    public function test_export_user_data_multiple_bkash_history() {
        $coursecontext2 = context_course::instance($this->course2->id);

        $this->setUser($this->student2);
        // Export all of the data for the context.
        $this->export_context_data_for_user($this->student2->id, $coursecontext2, 'enrol_bkash');
    ...
    public function test_delete_data_for_all_users_in_context() {
        ...
        // Before deletion, we should have 2 bkash transactions in course1 and 3 bkash transactions in course2.
        $this->assertEquals(
            2,
            $DB->count_records('enrol_bkash', ['courseid' => $this->course1->id])
        );
        $this->assertEquals(
            3,
            $DB->count_records('enrol_bkash', ['courseid' => $this->course2->id])
        );

        // Delete data based on context.
        provider::delete_data_for_all_users_in_context($coursecontext1);

        // After deletion, bkash transactions in course1 should have been deleted.
        $this->assertEquals(
            0,
            $DB->count_records('enrol_bkash', ['courseid' => $this->course1->id])
        );
    ...
    public function test_delete_data_for_all_users_in_context_multiple_transactions() {
        ...
        // After deletion, bkash transactions in course2 should have been deleted.
        $this->assertEquals(
            2,
            $DB->count_records('enrol_bkash', ['courseid' => $this->course1->id])
        );
    ...
    public function test_delete_data_for_user_from_single_context() {
        ...
        // Before deletion, we should have 2 bkash transactions (1 of them for student12) in course1
        // and 3 bkash transactions (1 of them for student12) in course2.
        $this->assertEquals(
            2,
            $DB->count_records('enrol_bkash', ['courseid' => $this->course1->id])
        );
        $this->assertEquals(
            1,
            $DB->count_records('enrol_bkash', ['courseid' => $this->course1->id, 'userid' => $this->student12->id])
        );
        $this->assertEquals(
            3,
            $DB->count_records('enrol_bkash', ['courseid' => $this->course2->id])
        ...
        $this->assertEquals(
            1,
            $DB->count_records('enrol_bkash', ['courseid' => $this->course2->id, 'userid' => $this->student12->id])
        );
    ...
    public function test_delete_data_for_user_for_business_user() {
        ...
        // Before deletion, we should have 5 bkash enrolments.
        // 3 of which paid to businessuser1 and 2 of which paid to businessuser2.
        $this->assertEquals(
            3,
            $DB->count_records('enrol_bkash', ['business' => $this->businessuser1->email])
        );
        $this->assertEquals(
            2,
            $DB->count_records('enrol_bkash', ['business' => $this->businessuser2->email])
        );
        ...
        $this->assertEquals(
            0,
            $DB->count_records('enrol_bkash', ['courseid' => $this->course1->id, 'business' => $this->businessuser1->email])
        );
        $this->assertEquals(
            2,
            $DB->count_records('enrol_bkash', ['business' => $this->businessuser1->email])
        ...
        $this->assertEquals(
            1,
            $DB->count_records('enrol_bkash', ['courseid' => $this->course1->id, 'business' => ''])
        );
        $this->assertEquals(
            2,
            $DB->count_records('enrol_bkash', ['business' => $this->businessuser2->email])
    ...
    public function test_delete_data_for_user_for_receiver_user() {
        ...
        $this->assertEquals(
            2,
            $DB->count_records('enrol_bkash', ['receiver_email' => $this->receiveruser1->email])
        );
        $this->assertEquals(
            3,
            $DB->count_records('enrol_bkash', ['receiver_email' => $this->receiveruser2->email])
        );
        ...
        $this->assertEquals(
            2,
            $DB->count_records('enrol_bkash', ['receiver_email' => ''])
        );
        $this->assertEquals(
            3,
            $DB->count_records('enrol_bkash', ['receiver_email' => $this->receiveruser2->email])
    ...
    public function test_get_users_in_context() {
        ...
        $userlist1 = new \core_privacy\local\request\userlist($coursecontext1, 'enrol_bkash');
        ...
        $userlist2 = new \core_privacy\local\request\userlist($coursecontext2, 'enrol_bkash');
        ...
        $userlist3 = new \core_privacy\local\request\userlist($coursecontext3, 'enrol_bkash');
    ...
    public function test_delete_data_for_users() {
        ...
        $this->assertEqualsCanonicalizing(
            ...
            $DB->get_fieldset_select('enrol_bkash', 'courseid', 'userid = ?', [$this->student12->id])
        );
        $this->assertEqualsCanonicalizing(
        ...
        // After deletion, bkash transactions for student12 in course1 should have been deleted.
        // Now, we should have 1 bkash transaction (which is not related to student12) in course1.
        // There should still be 3 bkash transactions (1 of them for student12) in course2.
    ...
    public function test_delete_data_for_users_business() {
        ...
        $this->assertEquals(3, $DB->count_records('enrol_bkash', ['business' => $this->businessuser1->email]));
        $transactions = $DB->get_records('enrol_bkash', [
        ...
        $this->assertEquals(2, $DB->count_records('enrol_bkash', ['business' => $this->businessuser1->email]));
        $this->assertEquals(0, $DB->count_records('enrol_bkash', [
            'courseid' => $this->course1->id,
            'business' => $this->businessuser1->email
        ]));
        ...
        $newtransaction = $DB->get_record('enrol_bkash', ['id' => $transaction->id]);
    ...
    public function test_delete_data_for_users_receiver() {
        ...
        $this->assertEquals(2, $DB->count_records('enrol_bkash', ['receiver_email' => $this->receiveruser1->email]));
        $transactions = $DB->get_records('enrol_bkash', [
            'courseid' => $this->course1->id,
        ...
        $this->assertEquals(0, $DB->count_records('enrol_bkash', ['receiver_email' => $this->receiveruser1->email]));
    ...
    protected function setUp(): void {
        ...
        $bkashplugin->enrol_user($enrolinstance1, $this->student1->id, $studentrole->id);
        $this->create_enrol_bkash_record(
            $this->businessuser1,
            $this->receiveruser1,
            $this->course1,
            $this->student1,
        ...
        $bkashplugin->enrol_user($enrolinstance2, $this->student2->id, $studentrole->id);
        ...
        // First in course1.
        $bkashplugin->enrol_user($enrolinstance1, $this->student12->id, $studentrole->id);
        $this->create_enrol_bkash_record(
            $this->businessuser2,
            $this->receiveruser1,
            $this->course1,
        ...
        // Then in course2.
        $bkashplugin->enrol_user($enrolinstance2, $this->student12->id, $studentrole->id);
        $this->create_enrol_bkash_record(
            $this->businessuser2,
            $this->receiveruser2,
            $this->course2,
        ...
        $bkashplugin->enrol_user($enrolinstance1, $this->student3->id, $studentrole->id);
        $this->create_enrol_bkash_record(
            $this->businessuser3,
            $this->receiveruser3,
            $this->course3,
    ...
    /**
     * Helper function to create an enrol_bkash record.
     *
     ...
     * @param String $txnid The bkash txnid to use
     * @param int $time The txn time
     */
    protected function create_enrol_bkash_record($business, $receiver, $course, $user, $enrol, $txnid, $time) {
        global $DB;
        ...
        $DB->insert_record('enrol_bkash', $bkashdata);
    }
}
